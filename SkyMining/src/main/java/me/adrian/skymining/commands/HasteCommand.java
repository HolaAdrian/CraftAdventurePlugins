...
public class HasteCommand implements CommandExecutor, TabCompleter {
    @Override
    public boolean onCommand(@NotNull CommandSender commandSender, @NotNull Command command, @NotNull String s, @NotNull String[] strings) {
        CommandSender player = commandSender;
        if (!(strings.length == 3)){
            player.sendMessage(ChatColor.RED + "Syntax: /haste <set/add> <player> <amount>");
            return false;
        }
            ...
            if (strings[0].equalsIgnoreCase("set")|| strings[0].equalsIgnoreCase("add")){
                if (Bukkit.getPlayer(strings[1]) != null){
                        ...
                        if (strings[0].contains("set")){
                            SkyMining.haste.put(Bukkit.getPlayer(strings[1]).getUniqueId(), amount);
                            player.sendMessage(ChatColor.GREEN + Bukkit.getPlayer(strings[1]).getName() + " hat jetzt " + amount + ". Haste!");
                        }
                        if (strings[0].contains("add")){
                            Integer i = SkyMining.haste.get(Bukkit.getPlayer(strings[1]).getUniqueId());
                            Integer ii = i + amount;
                            SkyMining.haste.put(Bukkit.getPlayer(strings[1]).getUniqueId(), ii);
                            player.sendMessage(ChatColor.GREEN + Bukkit.getPlayer(strings[1]).getName() + " hat jetzt " + ii + ". Haste!");
                        }
                }
                else {
